"""Simulate fermi-hubbard model using Ququarts."""
...
from scripts.primitives import *  # nopycln: import
...
class GammaHop(Gate):
    ...
    def _unitary_(self):
        a = np.array(exp(-1j * self.t * (self.J / (2j) * sy_gamma_5)))
        ...
        for it, t in enumerate(temps):
            ...
            if t != 0:
                ...
                for _ in range(steps_for_step * it):
                    ...
                    evolution_circuit.append(GammaHop(J, tau)(self.qubits[0]))
                    evolution_circuit.append(GammaHop(J, tau)(self.qubits[1]))
                    evolution_circuit.append(GammaHop(J, tau)(self.qubits[2]))
                    """
                    evolution_circuit.append(
            ...
            result = QuditResult(sample(circuit, repetitions=500))
...
def main():
    ...
    cols = int(input("Number of rows: "))

    qfh = QuditFermiHubbard(rows, cols)
